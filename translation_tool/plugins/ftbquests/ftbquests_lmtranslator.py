...
def translate_ftb_pending_to_zh_tw(
    *,
    ...
    if global_total_to_translate == 0:
        log_info(
            "ℹ️ [FTB-LM][狀態] 恭喜！所有內容皆命中快取，無需調用 AI，將直接導出檔案。"
        )
    ...
    for idx, (src, key_count) in enumerate(per_file_counts, start=1):
        ...
        file_hint = (
            f"config/ftbquests/quests/{rel_src}"  # ✅ include /ftbquests/ and no /lang/
        )
        ...
        if dry_run:
            ...
            if len(dry_preview_items) < 2000:
                dry_preview_items.extend(
                    items_to_translate[: 2000 - len(dry_preview_items)]
                )
        ...
        if not items_to_translate:
            ...
            progress_pct = (
                (overall_done / global_total_keys * 100) if global_total_keys > 0 else 0
            )
