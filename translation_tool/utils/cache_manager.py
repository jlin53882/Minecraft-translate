...
def save_translation_cache(cache_type: str, write_new_shard: bool = True):
    ...
    state = _state()
    with state.cache_lock:
        session_entries = cache_store.get_session_entries(state.session_new_entries, cache_type)
        if not session_entries:
            return
        data_to_save = cache_store.flush_session_entries(state.session_new_entries, cache_type)
        cache_store.clear_dirty(state.is_dirty, cache_type)
    ...
    try:
        ...
        if not save_path:
            return
        _save_entries_to_active_shards(cache_type, data_to_save, force_new_shard=write_new_shard)
    except Exception as e:
...
initialize_translation_cache()
_state_obj = _state()
log.info("快取統計：" + ", ".join(f"{k}={len(v)}" for k, v in _state_obj.translation_cache.items()))
