...
def check_english_residue_generator(
    ...
    if total_files == 0:
        yield {
            "progress": 1.0,
            "log": f"在 '{input_dir}' 中未找到任何 json 檔案。",
            "error": True,
        }
        return
    ...
    for json_file in json_files:
        ...
        if suspicious_entries:
            ...
            yield {
                "progress": progress,
                "log": f"在 {rel_path} 中找到 {len(suspicious_entries)} 個殘留英文條目。",
            }
        else:
            yield {"progress": progress, "log": f"檢查 {rel_path} ... OK。"}

    yield {"progress": 1.0, "log": "--- 殘留英文檢查完成 ---"}
    yield {
        "progress": 1.0,
        "log": f"總共在 {files_with_suspicious} 個檔案中，找到 {total_suspicious} 個可疑殘留英文條目。",
    }
