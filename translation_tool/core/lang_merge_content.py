        ...
        if os.path.exists(tw_output_path):
            try:
                ...
                if existing_data is not None:
                    # 標準化現有 TW JSON 格式
                    existing_normalized_bytes = json.dumps(
                        existing_data, option=json.OPT_INDENT_2
                    )
...
def _process_content_or_copy_file(
    ...
    if book_root:
        ...
        if has_cn_or_tw and "/en_us/" in normalized_path.lower():
            return {
                "success": True,
                "log": f"[Patchouli] 跳過已有翻譯的英文原件: {normalized_path}",
            }
        ...
        patchouli_root_dir = (
            matched_dir_name
            if isinstance(PATCHOULI_DIRS, list) and PATCHOULI_DIRS
            else PATCHOULI_DIRS
        )
        ...
        if has_cn_or_tw:
            ...
            target = os.path.join(
                output_dir, patchouli_root_dir, normalized_root, rel_path
            )
            action_log = "轉換中文化"
        ...
        # B) 沒中文化：輸出到 Patchouli/待翻譯/...
            ...
            target = os.path.join(
                output_dir, patchouli_root_dir, pending_name, normalized_root, rel_path
            )
            action_log = "歸檔至待翻譯"
    ...
    try:
            ...
            if ext == ".json":
                ...
                if os.path.exists(final_output_path):
                    try:
                        ...
                        existing_normalized_bytes = json.dumps(
                            existing_normalized_data, option=json.OPT_INDENT_2
                        )
            ...
            try:
                ...
                if not os.path.exists(final_output_path):
                    ...
                    with zf.open(input_path) as src:
                        # with open(final_output_path, 'wb') as dst:
                        with (
                            zf.open(input_path) as src,
                            open(final_output_path, "wb") as dst,
                        ):
                            shutil.copyfileobj(src, dst)
        ...
        # 2. JSON 檔案 (執行增量補缺)
            ...
            return _patch_localized_content_json(
                zf, input_path, final_output_path, rules, log_prefix, output_dir
            )
            ...
            if processor:
                ...
                if should_write:
                    ...
                    if is_forced_s2tw and not is_localized_cn_file:
                        log_msg = (
                            f"{log_prefix} 非本地化檔案 ({ext}) S2TW 結構化轉換完成。"
                        )
                    else:
