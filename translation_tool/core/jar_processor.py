...
def _extract_from_jar(
    jar_path: str, output_root: str, target_regex: re.Pattern
) -> Dict[str, Any]:
    """
...
def _run_extraction_process(
    ...
    if total_jars == 0:
        ...
        yield {
            "progress": 1.0,
        }  # "log": f"在 '{mods_dir}' 中未找到任何 .jar 檔案。"}
        return

    log.info(f"開始從 {total_jars} 個 .jar 檔案中提取 {process_name} 檔案...")
    yield {
        "progress": 0.0,
    }  # "log": f"開始從 {total_jars} 個 .jar 檔案中提取 {process_name} 檔案..."}

    processed_count = 0
    total_extracted = 0
    total_skipped = 0
    max_workers = (
        load_config().get("translation", {}).get("parallel_execution_workers")
        or os.cpu_count()
    )

    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_jar = {
            executor.submit(_extract_from_jar, jar, output_dir, target_regex): jar
        ...
        for future in concurrent.futures.as_completed(future_to_jar):
            ...
            try:
                ...
                yield {
                    "progress": progress,
                }  # "log": f"處理 {os.path.basename(jar_path)} 失敗: {exc}", "error": True}
...
def extract_book_files_generator(
    ...
    yield from _run_extraction_process(
        mods_dir, output_dir, book_path_regex, "Patchouli Book"
    )
...
def generate_preview_report(result: Dict[str, Any], mode: str, output_path: str) -> str:
    ...
    log.info(
        f"📊 統計摘要：找到 {total_files} 個檔案，來自 {len(preview_results)} 個 JAR，總大小 {total_size_mb:.2f} MB"
    )
