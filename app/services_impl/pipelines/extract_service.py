...
def _run_extraction(
    ...
    generator = extract_lang_files_generator(mods_dir, output_dir, lang_codes=lang_codes) if mode == 'lang' else extract_book_files_generator(mods_dir, output_dir, lang_codes=lang_codes)
...
def run_book_extraction_service(
    ...
    try:
        ...
        for update in extract_book_files_generator(mods_dir, output_dir, lang_codes=lang_codes):
            filtered: dict[str, Any] | None = GLOBAL_LOG_LIMITER.filter(update)
            if filtered is None:
                continue
