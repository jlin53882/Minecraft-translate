...
def run_generator_task(*, session, task_name: str, generator_factory: Callable[..., Iterable[Any]], kwargs: dict, add_session_log_on_error: bool = False, ui_log_handler=UI_LOG_HANDLER):
    """Run a generator task to exhaustion with shared lifecycle handling."""
    ensure_pipeline_logging()
    try:
        session.start()
        ui_log_handler.set_session(session)
        last = None
        for last in generator_factory(**kwargs):
            pass
        session.finish()
        return last
    except Exception as e:
