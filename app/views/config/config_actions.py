...
import traceback
from translation_tool.utils.config_manager import get_default, get_default_block
...
def load_config_into_view(view, config: dict):
    ...
    view.controls_map['logging.log_level'].value = log_cfg.get('log_level') or get_default('logging.log_level')
    view.controls_map['logging.log_dir'].value = log_cfg.get('log_dir') or get_default('logging.log_dir')
    view.controls_map['translator.output_dir_name'].value = trans_cfg.get('output_dir_name') or get_default('translator.output_dir_name')
    view.controls_map['ftb_translator.output_dir_name'].value = ftb_cfg.get('output_dir_name') or get_default('ftb_translator.output_dir_name')
    view.controls_map['translator.replace_rules_path'].value = trans_cfg.get('replace_rules_path') or get_default('translator.replace_rules_path')
    view.controls_map['translator.cache_directory'].value = trans_cfg.get('cache_directory') or get_default('translator.cache_directory')
    view.controls_map['translator.enable_cache_saving'].value = trans_cfg.get('enable_cache_saving')
    view.controls_map['translator.parallel_execution_workers'].value = str(trans_cfg.get('parallel_execution_workers') or get_default('translator.parallel_execution_workers'))
    view.controls_map['species_cache.cache_directory'].value = species_cfg.get('cache_directory') or get_default('species_cache.cache_directory')
    view.controls_map['species_cache.cache_filename'].value = species_cfg.get('cache_filename') or get_default('species_cache.cache_filename')
    ...
    view.controls_map['lm_translator.rate_limit.timeout'].value = str(lm_cfg.get('rate_limit', {}).get('timeout') or str(get_default('lm_translator.rate_limit.timeout')))
    view.controls_map['lm_translator.rate_limit.sleep_seconds_between_batches'].value = str(lm_cfg.get('rate_limit', {}).get('sleep_seconds_between_batches') or str(get_default('lm_translator.rate_limit.sleep_seconds_between_batches')))
    view.controls_map['output_bundler.output_zip_name'].value = bundle_cfg.get('output_zip_name')
    view.controls_map['lang_merger.pending_folder_name'].value = config.get('lang_merger', {}).get('pending_folder_name') or get_default('lang_merger.pending_folder_name')
    view.controls_map['lang_merger.pending_organized_folder_name'].value = config.get('lang_merger', {}).get('pending_organized_folder_name') or get_default('lang_merger.pending_organized_folder_name')
    view.controls_map['lang_merger.filtered_pending_min_count'].value = str(config.get('lang_merger', {}).get('filtered_pending_min_count') or get_default('lang_merger.filtered_pending_min_count'))
    view.controls_map['lm_translator.lm_translate_folder_name'].value = str(config.get('lm_translator', {}).get('lm_translate_folder_name') or get_default('lm_translator.lm_translate_folder_name'))
    view.controls_map['lm_translator.patchouli_system_prompt'].value = str(config.get('lm_translator', {}).get('patchouli_system_prompt') or get_default('lm_translator.patchouli_system_prompt'))
    view.controls_map['lm_translator.lang_system_prompt'].value = str(config.get('lm_translator', {}).get('lang_system_prompt') or get_default('lm_translator.lang_system_prompt'))
    view.controls_map['lang_merger.quarantine_folder_name'].value = config.get('lang_merger', {}).get('quarantine_folder_name') or get_default('lang_merger.quarantine_folder_name')
    ...
    view.controls_map['lm_translator.patchouli.dir_names'].value = '\n'.join(get_default('lm_translator.patchouli.dir_names'))
    view.controls_map['lm_translator.translator.skip_terms'].value = '\n'.join(get_default('lm_translator.translator.skip_terms'))
    view.controls_map['lm_translator.translator.translatable_keywords'].value = '\n'.join(get_default('lm_translator.translator.translatable_keywords'))

    extractor_cfg = config.get('extractor', {})
    folder_names = extractor_cfg.get('output_folder_names', {})
    view.controls_map['extractor.output_folder_names.lang_extract'].value = folder_names.get('lang_extract') or get_default('extractor.output_folder_names.lang_extract')
    view.controls_map['extractor.output_folder_names.book_extract'].value = folder_names.get('book_extract') or get_default('extractor.output_folder_names.book_extract')
    view.controls_map['extractor.output_folder_names.lang_preview'].value = folder_names.get('lang_preview') or get_default('extractor.output_folder_names.lang_preview')
    view.controls_map['extractor.output_folder_names.book_preview'].value = folder_names.get('book_preview') or get_default('extractor.output_folder_names.book_preview')
    view.controls_map['extractor.output_folder_names.dual_extract'].value = folder_names.get('dual_extract') or get_default('extractor.output_folder_names.dual_extract')
    view.controls_map['extractor.output_folder_names.dual_preview'].value = folder_names.get('dual_preview') or get_default('extractor.output_folder_names.dual_preview')
