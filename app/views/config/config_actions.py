...
def load_config_into_view(view, config: dict):
    ...
    view.controls_map['lang_merger.filtered_pending_min_count'].value = str(config.get('lang_merger', {}).get('filtered_pending_min_count', 3))
    pending_name = config.get('lang_merger', {}).get('pending_folder_name', '待翻譯')
    organized_name = config.get('lang_merger', {}).get('pending_organized_folder_name', '待翻譯整理需翻譯')
    min_count = config.get('lang_merger', {}).get('filtered_pending_min_count', 3)
    ...
    view.controls_map['lang_merger.quarantine_folder_name'].value = config.get('lang_merger', {}).get('quarantine_folder_name', '問題檔案skipped_json')
