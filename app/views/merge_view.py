...
class MergeView(ft.Column):
            ...
            self.zip_list_view.controls.append(
                ft.Row(
                    ...
                    controls=[
                        ...
                        ft.IconButton(icon=ft.Icons.CLOSE, tooltip="移除", on_click=lambda ev: self._remove_zip(path)),
                    ],
                )
            )
    ...
    def _show_merge_summary(self) -> None:
        """顯示合併完成摘要對話框（輸出統計 + 失敗 ZIP 列表）。"""
        summary: dict[str, Any] = getattr(self.session, "summary", {})
        failed_zips: list[dict[str, Any]] = summary.get("failed_zips", [])
        oc: dict[str, Any] = summary.get("output_counts", {})

        # 建立輸出統計 rows
        oc_rows: list[ft.Text] = []
        if oc:
            for label, count in [
        ...
        # 建立失敗 ZIP rows
        fail_rows: list[ft.Text] = []
        if failed_zips:
            for item in failed_zips:
                fail_rows.append(ft.Text(f"├─ {item['name']}", size=13))
                fail_rows.append(ft.Text(f"│  └─ 原因：{item['reason']}", size=12, color=theme.RED_400))

        # 組合內容
        content_controls: list[ft.Control] = []
        if oc_rows:
            content_controls.append(ft.Text("📁 輸出統計", size=14, weight=ft.FontWeight.BOLD))
            content_controls.extend(oc_rows)
        if fail_rows:
            if content_controls:
                content_controls.append(ft.Divider())
            content_controls.append(ft.Text("⚠️ 失敗 ZIP", size=14, weight=ft.FontWeight.BOLD, color=theme.RED_500))
            content_controls.extend(fail_rows)
        if not content_controls:
            content_controls.append(ft.Text("無輸出統計資料", size=13, color=theme.GREY_500))

        dlg = ft.AlertDialog(
            modal=True,
            title=ft.Text("合併完成摘要"),
            content=ft.Container(
                width=400,
                content=ft.Column(content_controls, spacing=6),
            ),
            actions=[
                ft.TextButton("關閉", on_click=lambda ev: self._close_summary_dialog(dlg)),
            ],
        )
        self.page.dialog = dlg
        dlg.open = True

    def _close_summary_dialog(self, dlg: ft.AlertDialog) -> None:
        """關閉摘要對話框。"""
        dlg.open = False
        self.page.update()
