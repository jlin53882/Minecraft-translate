...
class QCView(ft.Column):
    ...
    def __init__(self, page: ft.Page, file_picker: ft.FilePicker):
        ...
        # --- 「未翻譯檢查」的 UI 元件 ---
        self.en_dir_textfield = ft.TextField(label="英文 (en_us) 來源資料夾", expand=True)
        self.tw_dir_textfield_1 = ft.TextField(label="繁中 (zh_tw) 來源資料夾 (未翻譯檢查)", expand=True)
        self.untranslated_out_dir_textfield = ft.TextField(label="未翻譯報告 輸出資料夾", expand=True)
        ...
        self.cn_dir_textfield = ft.TextField(label="簡中 (zh_cn) 來源資料夾 (JSON)", expand=True)
        self.tw_dir_textfield_2 = ft.TextField(label="繁中 (zh_tw) 來源資料夾 (JSON)", expand=True)
        self.compare_out_dir_textfield = ft.TextField(label="JSON 差異報告 輸出資料夾", expand=True)
        ...
        self.tsv_file_textfield = ft.TextField(label="簡繁差異 TSV 檔案路徑", expand=True)
        self.tsv_out_file_textfield = ft.TextField(label="TSV 差異報告 輸出檔案 (.csv)", expand=True)
        ...
        self.controls = [
            ...
            ft.Card(content=ft.Container(padding=15, content=ft.Column([
                ft.Text("翻譯 Key 缺失檢查 (en_us vs zh_tw)", style=ft.TextThemeStyle.TITLE_LARGE),
                ft.Row([self.en_dir_textfield, self._create_pick_button(self.en_dir_textfield, "選擇英文 (en_us) 來源資料夾", folder_mode=True)]),
                ft.Row([self.tw_dir_textfield_1, self._create_pick_button(self.tw_dir_textfield_1, "選擇繁中 (zh_tw) 來源資料夾", folder_mode=True)]),
                ft.Row([self.untranslated_out_dir_textfield, self._create_pick_button(self.untranslated_out_dir_textfield, "選擇報告輸出資料夾", folder_mode=True)]),
                self.untranslated_start_button
            ], spacing=15))),
            
            # 卡片 2: 簡繁差異比較 - JSON 資料夾模式 (variant_comparator.py)
            ft.Card(content=ft.Container(padding=15, content=ft.Column([
                ft.Text("簡繁翻譯差異比較 - JSON 資料夾模式", style=ft.TextThemeStyle.TITLE_LARGE),
                ft.Text("適用於大規模翻譯資料夾的比對，輸出 JSON 報告。", style=ft.TextThemeStyle.BODY_SMALL, color=ft.Colors.BLUE_GREY),
                ft.Row([self.cn_dir_textfield, self._create_pick_button(self.cn_dir_textfield, "選擇簡中 (zh_cn) 來源資料夾", folder_mode=True)]),
                ft.Row([self.tw_dir_textfield_2, self._create_pick_button(self.tw_dir_textfield_2, "選擇繁中 (zh_tw) 來源資料夾", folder_mode=True)]),
                ft.Row([self.compare_out_dir_textfield, self._create_pick_button(self.compare_out_dir_textfield, "選擇 JSON 報告輸出資料夾", folder_mode=True)]),
                self.compare_start_button
            ], spacing=15))),

            # 卡片 3: 簡繁差異比較 - TSV 單檔案模式 (compare_zh_variants.py)
            ft.Card(content=ft.Container(padding=15, content=ft.Column([
                ft.Text("簡繁翻譯差異比較 - TSV 單檔案模式", style=ft.TextThemeStyle.TITLE_LARGE),
                ft.Text("比較 TSV 檔案中 'zh_cn' 和 'zh_tw' 欄位的差異。將 'zh_cn' 轉換為繁體中文後，與 'zh_tw' 進行比較，並列出所有不匹配的條目。", style=ft.TextThemeStyle.BODY_SMALL, color=ft.Colors.BLUE_GREY),
                ft.Row([self.tsv_file_textfield, self._create_pick_button(self.tsv_file_textfield, "選擇 TSV 檔案", folder_mode=False, file_filter="TSV files (*.tsv)")]),
                ft.Row([self.tsv_out_file_textfield, self._create_pick_button(self.tsv_out_file_textfield, "選擇 CSV 輸出檔案", folder_mode=False, file_filter="CSV files (*.csv)")]),
    ...
    def _create_pick_button(self, target_textfield: ft.TextField, title: str, folder_mode: bool, file_filter: str = None):
        ...
        return ft.IconButton(
            ...
            tooltip=title,
            on_click=lambda e: self.pick_file_or_directory_with_tkinter(e, target_textfield, title, folder_mode, file_filter)
        )
        ...
        try:
            ...
            if folder_mode:
                ...
                if 'CSV' in file_filter or 'JSON' in file_filter: 
                    path = filedialog.asksaveasfilename(title=title, defaultextension=file_filter.split('(')[1].strip(')').replace('*', '.'), filetypes=[(file_filter.split('(')[0].strip(), file_filter.split('(')[1].strip(')'))])
                # 輸入檔案用 askopenfilename
                else: 
                    path = filedialog.askopenfilename(title=title, filetypes=[(file_filter.split('(')[0].strip(), file_filter.split('(')[1].strip(')'))])
    ...
    def set_controls_disabled(self, disabled: bool):
        ...
        controls_to_disable = [
            ...
            self.untranslated_start_button, 
            # JSON 比較
            self.cn_dir_textfield, self.tw_dir_textfield_2, self.compare_out_dir_textfield, 
            self.compare_start_button,
            # TSV 比較
            self.tsv_file_textfield, self.tsv_out_file_textfield, self.compare_tsv_start_button
        ]
    ...
    def start_task(self, task_type: str):
        ...
        # 2. JSON 資料夾差異比較
            ...
            if not cn_dir or not tw_dir or not out_dir:
                ...
                self.set_controls_disabled(False)
                return
            self.log_view.controls.append(ft.Text("[系統] 開始執行 JSON 資料夾簡繁差異比較..."))
        ...
        # 3. TSV 單檔案差異比較 (新增)
            ...
            if not tsv_path or not out_csv_path:
                ...
                self.set_controls_disabled(False)
                return
            self.log_view.controls.append(ft.Text("[系統] 開始執行 TSV 單檔案簡繁差異比較..."))
