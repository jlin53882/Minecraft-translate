...
class ExtractorView(ft.Column):
    ...
    def _build_logs_card(self) -> ft.Container:
        """构建日志卡片 UI 组件"""
    ...
    # UI helpers
    # ==================================================
    def _pick_button(self, target: ft.TextField) -> ft.ElevatedButton:
        """构建目录选择按钮"""
        return build_pick_button(self, target)

    def pick_directory(self, target: ft.TextField) -> None:
        """開啟目錄選擇對話框"""
    ...
    def _on_dir_picked(self, e: ft.FilePickerResultEvent, target: ft.TextField) -> None:
        """處理目錄選擇結果"""
        if e.path:
            target.value = e.path
            self.page.update()
    ...
    def clear_output_path(self, e: ft.ControlEvent | None = None) -> None:
        """清除輸出路徑欄位"""
    ...
    # ==================================================
    def _start_ui_poller(self, mode: str = "") -> None:
        """启动 UI 轮询器以定期更新界面状态"""
        return run_ui_poller(self, mode=mode)

    def _update_stats_from_log(self, line: str) -> None:
        """根据日志内容更新提取统计信息"""
        return update_stats_from_log(self, line)

    def _show_extraction_summary(self, mode: str) -> None:
    ...
    def _append_log_line(self, entry_or_str: Any) -> None:
        """新增日誌訊息到日誌檢視區。
    ...
    # Worker Logic
    # ==================================================
    def start_extraction(self, mode: str) -> None:
        """启动 JAR 文件提取任务（lang 或 book 模式）"""
        return run_extraction_flow(self, mode)

    def _show_snack_bar(self, message: str, color: str = theme.RED_400) -> None:
        """
        顯示底部的快訊通知 (SnackBar)
    ...
    # 預覽功能
    # ==================================================
    def show_preview(self, mode: str) -> None:
        """显示提取预览对话框（lang 或 book 模式）"""
        return run_preview_flow(self, mode)

    def _show_preview_dialog_result_v2(self, result: dict[str, Any], mode: str) -> None:
        """显示预览结果对话框"""
    ...
    def _show_preview_dialog_error_v2(self, error: str, mode: str) -> None:
        """显示预览错误对话框"""
    ...
    def _start_from_preview_overlay(self, dialog: ft.AlertDialog, mode: str) -> None:
