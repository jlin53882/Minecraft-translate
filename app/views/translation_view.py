...
class TranslationView(ft.Column):
    ...
    def _action_row(
        ...
        *,
        ...
        on_dry_run: Callable[[ft.ControlEvent], None],
    ...
    # directory picker
    # ------------------------------------------------------------------
    def _pick_directory_into(self, target: ft.TextField) -> None:
        """開啟目錄選擇器並設定目標欄位"""
    ...
    # runners
    # ------------------------------------------------------------------
    def _run_ftb(self, *, dry_run: bool) -> None:
        """執行 FBT 翻譯流程"""
        return run_ftb(self, dry_run=dry_run)

    def _run_kjs(self, *, dry_run: bool) -> None:
        """執行 KubeJS 翻譯流程"""
        return run_kjs(self, dry_run=dry_run)

    def _run_md(self, *, dry_run: bool) -> None:
        """執行 Markdown 翻譯流程"""
    ...
    # reset actions
    # ------------------------------------------------------------------
    def _reset_ftb_inputs(self) -> None:
        """重置 FTB 翻譯的所有輸入欄位"""
    ...
    def _reset_kjs_inputs(self) -> None:
        """重置 KubeJS 翻譯的所有輸入欄位"""
    ...
    def _reset_md_inputs(self) -> None:
    ...
    def _show_snack(self, message: str, color: str = theme.RED_600) -> None:
