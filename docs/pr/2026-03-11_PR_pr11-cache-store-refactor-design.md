...
- `cache_manager.py` 內部改用 store 層處理 state/store CRUD，但保留 façade + orchestration
- 新增 `tests/test_cache_store.py`

### 實作原則是否符合設計
- ✅ `_cache_lock` 仍留在 `cache_manager.py`
- ✅ `_translation_cache/_initialized` ownership 未搬移
- ✅ 未新增 `cache_bootstrap.py`
- ✅ 未改 search API contract
- ✅ `cache_store.py` 維持非 thread-safe 純操作層，由 manager 負責 synchronization

## 驗證結果（實測）
- `uv run pytest tests/test_cache_store.py -q` → **3 passed**
...
- `uv run python -c "from translation_tool.core import lm_translator; print('lm import ok')"` → **lm import ok**
- add/save/reload smoke 已納入 `tests/test_cache_store.py::test_manager_add_save_reload_smoke` 並通過

## 風險 / 注意事項（落地後）
- `cache_store.py` 目前是非 thread-safe 純操作層；未來若有人繞過 `cache_manager` 直接操作 store，仍需自行處理同步責任。
- `add_entry(...)` 保留既有相容語意：只有 `dst` 變化才視為更新，避免不必要覆寫。
