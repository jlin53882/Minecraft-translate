...
- `translation_tool/utils/cache_search.py`
  - 新增並收斂 metadata helper：
    - `_extract_path_from_composite_key`
    - `_infer_search_path`
    - `_infer_search_mod`
    - `_build_search_metadata`
  - 新增 orchestration helper：
    - `build_index_entries(cache_type, cache_dict)`
    - `rebuild_from_cache_dicts(engine, cache_types, cache_state)`
  - 新增 `SearchOrchestrator`，負責 engine lifecycle、full rebuild、single-type rebuild、search query façade
- `translation_tool/utils/cache_manager.py`
  - 對外 search API 保持不變
  - 內部改為委派給 `cache_search.SearchOrchestrator`
- 新增 `tests/test_cache_search_orchestration.py`

### 實作原則是否符合設計
- ✅ 走 PR11 的 `cache_store` accessor
- ✅ 未新增 `cache_metadata.py`
- ✅ 索引生命週期採「先建後切換」
- ✅ search API contract / UI 相容未破

## 驗證結果（實測）
- `uv run pytest -q` → **37 passed**
- `uv run python -c "from translation_tool.utils import cache_manager; print('import ok')"` → **import ok**
- `uv run python -c "from app.services import cache_search_service; print('service import ok')"` → **service import ok**
- rebuild 後 schema 欄位相容 smoke → **fields_ok True**（`key/src/dst/mod/path/type` 皆存在）
- `rebuild_search_index_for_type` 不污染其他 type → **type_isolated True**
- 連續兩次 rebuild 不留孤立 `.db/.tmp` → **double_rebuild_clean True db 1 tmp 0**
- rebuild 期間 query 行為可預期（至少不 crash）→ **rebuild_query_predictable True**

## 風險 / 注意事項（落地後）
- 在 Windows 下，為了能 `os.replace` 切換索引檔，會先 close 舊 engine 再掛新 engine；這段極短窗口內，同時查詢可能拿到空/重試，但目前測試證明至少不 crash。
- `cache_search.py` 的 LF/CRLF 警告屬 Git 行尾提示，功能不受影響。
