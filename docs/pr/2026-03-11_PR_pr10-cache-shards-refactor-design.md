...
## Follow-up（語意清晰化）
後續已再補一輪小修，僅做可讀性與意圖說明，不改邏輯：
- 為 `_write_json_atomic` wrapper 補 docstring，說明目前雖回傳 `None`，但刻意透傳底層回傳值以保留未來擴充空間
- 為 shard helper 的 `logger` 參數補 type hint
- 在 `force_new_shard` 路徑補註解，說明先 ensure `.active` 再直接讀取的意圖
- 在 while loop 尾端的 rotate 呼叫補 comment 與 `_ = ...`，明示這是 pre-rotate 且刻意忽略回傳值
- 驗證：`uv run pytest tests/test_cache_shards.py -q`、`uv run pytest -q` 皆通過
